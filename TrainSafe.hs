...
makeSafe t (Speed, m) | state sec == Justleft = makeSafe t (Sensor, (SensorMessage {upd=Low,updid=(sid sec)}))
					  | otherwise = makeSafe t (Sensor, SensorMessage {upd=Hi,updid=(sid sec)})
	where sec = getSectionBySlot t (fromslot m)
makeSafe t (Direction, m) | state sec == Justleft = makeSafe t (Sensor, (SensorMessage {upd=Low,updid=(sid sec)}))
						  | otherwise = makeSafe t (Sensor, SensorMessage {upd=Hi,updid=(sid sec)})
